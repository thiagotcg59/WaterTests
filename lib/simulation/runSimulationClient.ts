...
export interface TimeSeriesData {
  ...
  links: Record<string, { flow: number[]; velocity: number[]; headloss: number[] }>;
}
...
export interface RunSimulationFailure {
  success: false;
  error: string;
  errorHint?: string;
...
export async function runSimulationClient(input: RunSimulationInput): Promise<RunSimulationResult> {
  ...
  const customPattern = Array.isArray(input?.consumptionPattern) && input.consumptionPattern.length === 24
    ? input.consumptionPattern : null;

  if (!inp) {
    return { success: false, error: 'Conteúdo do arquivo INP é obrigatório.' };
  }
  ...
  try {
    ...
    if (durationHours > 0) {
      ...
      const patternToUse = customPattern || [
        ...
        0.8, 1.3, 1.5, 1.3, 1.1, 1.0,
        1.1, 1.0, 0.9, 0.9, 1.0, 1.2,
